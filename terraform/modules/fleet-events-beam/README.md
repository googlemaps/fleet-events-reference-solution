...
<UPDATE main.tf to match your environment>
terraform init 
terraform plan
terraform apply
```
...
| <a name="input_FLEETEVENTS_FUNCTION_NAME"></a> [FLEETEVENTS\_FUNCTION\_NAME](#input\_FLEETEVENTS\_FUNCTION\_NAME) | sample function to run. | `string` | `"TASK_OUTCOME"` | no |
...
| <a name="input_FLEETEVENTS_WINDOW_SIZE"></a> [FLEETEVENTS\_WINDOW\_SIZE](#input\_FLEETEVENTS\_WINDOW\_SIZE) | Window size to use to process events, in minutes. This parameter does not apply to VEHICLE\_OFFLINE jobs. | `number` | `3` | no |
...
| <a name="input_PIPELINE_NAME"></a> [PIPELINE\_NAME](#input\_PIPELINE\_NAME) | Name of the Beam Pipeline. This will become the prefix of the Dataflow/Beam pipeline Job. | `string` | n/a | yes |
