...
/**
 * A utility class to externalize utility methods for better readability and
 * reusability
 */
public class Util {

    record CloudEventPubsubBody(
            CloudEventPubsubMessage message) {
    }
    ...
    static void log(Logger LOGGER, CloudEventPubsubMessage msg) {
        ...
        msg.attributes().keySet().stream().sorted().forEach((key) -> {
    ...
    static void log(Logger LOGGER, LogEntry logentry) {
        ...
        logentry.getLabelsMap().keySet().forEach((key) -> {
            LOGGER.info(String.format("%s.labels[%s] : %s", PREFIX, key, logentry.getLabelsMap().get(key)));
        });
        LOGGER.info(String.format("%s.resource.type : %s", PREFIX, logentry.getResource().getType()));

        logentry.getResource().getLabelsMap().keySet().forEach((key) -> {
            LOGGER.info(String.format(
                    "%s.resource.labels[%s] : %s",
                    PREFIX,
                    key,
                    logentry.getResource().getLabelsMap().get(key)));
    ...
    static void log(Logger LOGGER, AuditLog log) {
        ...
        LOGGER.info(String.format("%s.authenticationInfo.principalEmail : %s", PREFIX,
                log.getAuthenticationInfo().getPrincipalEmail()));
    }

    // building on top of sample here
    // https://cloud.google.com/pubsub/docs/samples/pubsub-publish-custom-attributes
    static void publishFleetEventMessage(Publisher publisher, FleetEvents.FleetEventMessage msg, Logger LOGGER)
    ...
    static FleetEvents.FleetEventMessage logentry2fleeteventmsg(LogEntry logentry) {
        ...
        FleetEvents.FleetEventMessage msg = new FleetEvents.FleetEventMessage(
                eventname,
                usecase,
                msg_attributes,
                eventtime,
